...
import { IBlogPost, BlogComment, BlogImage, BlogFeaturedImage } from '../types/index';
...
BlogPostSchema.virtual('approvedComments').get(function(this: IBlogPost): BlogComment[] {
...
// Interface for published posts options
...
BlogPostSchema.statics.getPublishedPosts = function(options: PublishedPostsOptions = {}) {
  const {
    page = 1,
    limit = 10,
    category,
...
BlogPostSchema.statics.getRelatedPosts = async function(postId: string, limit: number = 5): Promise<IBlogPost[]> {
...
BlogPostSchema.methods.incrementViewCount = function(this: IBlogPost): Promise<IBlogPost> {
  this.viewCount += 1;
  return this.save();
};

// Method to add comment
BlogPostSchema.methods.addComment = function(this: IBlogPost, commentData: Partial<BlogComment>): Promise<IBlogPost> {
  this.comments.push(commentData as BlogComment);
  return this.save();
};

// Method to moderate comment
BlogPostSchema.methods.moderateComment = function(this: IBlogPost, commentId: string, status: 'pending' | 'approved' | 'rejected', moderatorId: string): Promise<IBlogPost> {
  const comment = this.comments.find(c => c.id === commentId);
  if (comment) {
    comment.status = status;
    comment.moderatedBy = new mongoose.Types.ObjectId(moderatorId);
...
BlogPostSchema.set('toJSON', {
  transform: function(doc: any, ret: any) {
    ...
    if (ret.comments) {
      ret.comments = ret.comments.filter((comment: BlogComment) => comment.status === 'approved');
    }
    return ret;
  }
});
...
export default mongoose.model<IBlogPost, IBlogPostModel>('BlogPost', BlogPostSchema); 