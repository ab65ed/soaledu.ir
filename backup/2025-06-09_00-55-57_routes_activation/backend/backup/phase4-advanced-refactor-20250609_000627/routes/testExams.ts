...
} = require('../validations/testExamValidation');
...
router.post('/:id/generate-questions', 
  authenticateUser, 
  validateExamConfiguration,
  TestExamController.generateQuestions
);
...
router.post('/sessions/:sessionId/answer', 
  authenticateUser, 
  TestExamController.submitAnswer
);
 ...
 * @access  Private (participant only)
 */
router.post('/sessions/:sessionId/finish', 
  authenticateUser, 
  TestExamController.finishExam
);
...
router.get('/my/sessions', 
  ...
  async (req, res) => {
    try {
      ...
      if (status) totalQuery.equalTo('status', status);
