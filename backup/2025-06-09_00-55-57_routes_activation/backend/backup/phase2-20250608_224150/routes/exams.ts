...
const Exam = require('../models/exam.model') as any; 
const Question = require('../models/question.model') as any;
const User = require('../models/user.model') as any;
...
router.post('/create', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const {
      ...
      category,
      lesson,
      questions, // Expecting array of question IDs
      hasNegativeMarking,
...
router.get('/admin/all', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
    if (!req.user || !['admin'].includes(req.user.role)) {
      res.status(403).json({ 
        success: false, 
...
router.get('/available', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
...
router.get('/:id', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (req.user?.role === 'student' && !isCreator && exam.questions && Array.isArray(exam.questions)) {
      exam.questions = exam.questions.map((q: any) => q.getForDisplay ? q.getForDisplay() : q); // Check if method exists
    }
...
router.patch('/:id/publish', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
    const examId = req.params.id;
    const exam = await Exam.findById(examId as any);
    if (!exam) { res.status(404).json({ success: false, message: 'آزمون پیدا نشد' }); return; }

    if (!req.user) { res.status(401).json({ success: false, message: "Unauthorized" }); return;}
    const isCreator = exam.creator?.toString() === req.user.id;
    const isAdmin = req.user.role === 'admin';
    if (!isCreator && !isAdmin) { res.status(403).json({ success: false, message: 'دسترسی غیرمجاز' }); return; }
    if (exam.status !== 'draft') { res.status(400).json({ success: false, message: 'فقط آزمون‌های پیش‌نویس قابل انتشار هستند' }); return; }

    if (typeof exam.publish !== 'function') throw new Error('exam.publish is not a function');
    await exam.publish();
...
router.patch('/:id/schedule', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { startDate, endDate } = req.body;
    if (!startDate || !endDate) { res.status(400).json({ success: false, message: 'تاریخ شروع و پایان الزامی است' }); return; }
    ...
    const isAdmin = req.user.role === 'admin';
    if (!isCreator && !isAdmin) { res.status(403).json({ success: false, message: 'دسترسی غیرمجاز' }); return; }
    if (!['draft', 'published'].includes(exam.status)) { res.status(400).json({ success: false, message: 'فقط آزمون‌های پیش‌نویس یا منتشر شده قابل زمان‌بندی هستند' }); return; }

    const start = new Date(startDate);
    const end = new Date(endDate);
    if (start <= new Date()) { res.status(400).json({ success: false, message: 'تاریخ شروع باید در آینده باشد' }); return; }
    if (end <= start) { res.status(400).json({ success: false, message: 'تاریخ پایان باید بعد از تاریخ شروع باشد' }); return; }

    if (typeof exam.schedule !== 'function') throw new Error('exam.schedule is not a function');
...
router.patch('/:id/archive', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
    const examId = req.params.id;
    const exam = await Exam.findById(examId as any);
    if (!exam) { res.status(404).json({ success: false, message: 'آزمون پیدا نشد' }); return; }
    ...
    if (typeof exam.archive !== 'function') throw new Error('exam.archive is not a function');
    await exam.archive();
...
router.delete('/:id', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
    if (req.user?.role !== 'admin') { res.status(403).json({ success: false, message: 'فقط ادمین می‌تواند آزمون را حذف کند' }); return; }

    const examId = req.params.id;
    const exam = await Exam.findById(examId as any);
    if (!exam) { res.status(404).json({ success: false, message: 'آزمون پیدا نشد' }); return; }
    if (['active', 'completed'].includes(exam.status)) { res.status(400).json({ success: false, message: 'نمی‌توان آزمون فعال یا تکمیل شده را حذف کرد' }); return; }

    await Exam.findByIdAndDelete(examId as any);
...
router.post('/:id/start', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
    const examId = req.params.id;
    const examTemplate = await Exam.findById(examId as any);
    if (!examTemplate) { res.status(404).json({ success: false, message: 'آزمون پیدا نشد' }); return; }
    ...
    if (existingSession) { res.status(400).json({ success: false, message: 'شما قبلاً این آزمون را شروع کرده‌اید' }); return; }
    ...
    if (typeof examSession.start !== 'function') throw new Error('examSession.start is not a function');
    ...
    if (Array.isArray(examSession.questions)) {
        examSession.questions = examSession.questions.map((q: any) => q.getForDisplay ? q.getForDisplay() : q);
    }
...
router.post('/:id/submit', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { answers } = req.body as { answers: { [key: string]: any } }; // Assuming answers is an object: { questionId: userAnswer }
    const examSessionId = req.params.id;

    const examSession = await Exam.findOne({ _id: examSessionId as any, user: req.user?.id, status: 'active' }).populate('questions');
    if (!examSession) { res.status(404).json({ success: false, message: 'جلسه آزمون پیدا نشد' }); return; }

    if (typeof examSession.isTimedOut !== 'function' || examSession.isTimedOut()) {
      if (typeof examSession.expire === 'function') await examSession.expire();
    ...
    if (!Array.isArray(examSession.questions)) throw new Error('examSession.questions is not an array');
    const totalQuestions = examSession.questions.length;

    const results = examSession.questions.map((question: any) => {
      const userAnswer = answers[question._id.toString()];
      if (typeof question.checkAnswer !== 'function') throw new Error(`question.checkAnswer is not a function for question ID: ${question._id}`);
    ...
    const maxPossibleScore = examSession.questions.reduce((sum: number, q: any) => sum + (q.points || 1), 0);
    const percentage = maxPossibleScore > 0 ? (totalScore / maxPossibleScore) * 100 : 0;
    const passed = percentage >= examSession.passingScore;

    if (typeof examSession.complete !== 'function') throw new Error('examSession.complete is not a function');
...
router.post('/:id/auto-save', auth, async (req: RequestWithUser, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { answers } = req.body;
    const examSessionId = req.params.id;
    const examSession = await Exam.findOne({ _id: examSessionId as any, user: req.user?.id, status: 'active' });
