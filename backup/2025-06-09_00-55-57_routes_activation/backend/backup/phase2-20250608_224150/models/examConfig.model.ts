...
export interface IExamConfig extends Document {
  ...
  timeWarnings: number[]; // warning times in minutes before exam ends
  autoSubmit: boolean; // auto submit when time is up
  
  // Question settings
  shuffleQuestions: boolean;
  shuffleOptions: boolean;
  questionsPerPage: number;
  allowReview: boolean; // allow reviewing answered questions
  showQuestionNumbers: boolean;
  
  // Navigation settings
  allowBackward: boolean; // allow going back to previous questions
  allowForward: boolean; // allow skipping questions
  showProgress: boolean; // show progress bar
  
  // Security settings
  lockdownBrowser: boolean; // require lockdown browser
  preventCopyPaste: boolean;
  preventRightClick: boolean;
  preventScreenshots: boolean;
  tabSwitchLimit: number; // max number of allowed tab switches
  
  // Scoring settings
  negativeMarking: boolean;
  negativeMarkingValue: number; // points deducted for wrong answers
  partialMarking: boolean; // allow partial marks
  passingScore: number; // minimum score to pass (percentage)
  
  // Display settings
  showResults: boolean; // show results after exam
  showCorrectAnswers: boolean; // show correct answers after exam
  showExplanations: boolean; // show explanations after exam
  showScore: boolean; // show score immediately after exam
  
  // Attempts settings
  maxAttempts: number; // maximum number of attempts allowed
  retakeDelay: number; // delay in hours before retake is allowed
...
const examConfigSchema = new Schema<IExamConfig>(
  {
    ...
    timeWarnings: {
      ...
      validate: {
        validator: function(warnings: number[]) {
...
const ExamConfig = mongoose.model<IExamConfig>('ExamConfig', examConfigSchema);
