...
export interface IScore extends Document {
  ...
  bonusPoints: number; // any bonus points awarded
  ...
  suspiciousActivity: string[]; // array of suspicious activities detected
  violations: number; // number of violations (if any)
  ...
  // Analytics data
  difficultyBreakdown: {
    easy: { total: number; correct: number; points: number };
    medium: { total: number; correct: number; points: number };
    hard: { total: number; correct: number; points: number };
  };
  ...
  // Additional metadata
  examVersion: number; // version of exam when taken
...
// Static methods
scoreSchema.statics.getExamStatistics = function(examId: string) {
  return this.aggregate([
    { $match: { exam: new mongoose.Types.ObjectId(examId), isCompleted: true } },
    {
      $group: {
        _id: null,
        totalAttempts: { $sum: 1 },
...
scoreSchema.statics.getUserBestScore = function(userId: string, examId: string) {
  return this.findOne({
    user: userId,
    exam: examId,
    isCompleted: true
...
scoreSchema.statics.calculateScore = async function(examId: string, userId: string) {
...
const Score = mongoose.model<IScore, IScoreModel>('Score', scoreSchema);
