...
// Middleware functions for Express validation
...
export const validateQuestionCreate = (req: Request, res: Response, next: NextFunction) => {
  try {
    QuestionCreateSchema.parse(req.body);
    next();
  } catch (error) {
...
export const validateQuestionUpdate = (req: Request, res: Response, next: NextFunction) => {
  try {
    QuestionUpdateSchema.parse({ ...req.body, id: req.params.id });
    next();
  } catch (error) {
    if (error instanceof z.ZodError) {
      res.status(400).json({
 ...
 */
export const validateQuestionAutoSave = (req: Request, res: Response, next: NextFunction) => {
  try {
    QuestionAutoSaveSchema.parse({ ...req.body, id: req.params.id });
    next();
  } catch (error) {
    if (error instanceof z.ZodError) {
      res.status(400).json({
 ...
 */
export const validateQuestionSearch = (req: Request, res: Response, next: NextFunction) => {
  try {
    QuestionSearchSchema.parse(req.query);
    next();
  } catch (error) {
    if (error instanceof z.ZodError) {
      res.status(400).json({
