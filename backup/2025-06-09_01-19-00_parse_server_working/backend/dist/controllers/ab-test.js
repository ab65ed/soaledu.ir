...
exports.deleteABTest = exports.updateABTest = exports.getABTestAnalytics = exports.getABTestResults = exports.recordConversion = exports.assignUserToVariant = exports.stopABTest = exports.pauseABTest = exports.startABTest = exports.getABTestById = exports.createABTest = exports.getABTests = void 0;
...
const createABTest = async (req, res) => {
    try {
        ...
        const totalPercentage = testData.variants.reduce((sum, v) => sum + v.percentage, 0);
 ...
 */
const getABTestById = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        // محاسبه آمار اضافی
        const testParticipants = participants.filter(p => test.variants.some(v => v.id === p.variantId));
...
exports.getABTestById = getABTestById;
/**
 * شروع تست A/B
 */
 ...
 * متوقف کردن تست A/B
 */
const pauseABTest = async (req, res) => {
    try {
        const { id } = req.params;
        const test = abTests.find(t => t.id === id);
        if (!test) {
 ...
 * پایان تست A/B
 */
const stopABTest = async (req, res) => {
    try {
        const { id } = req.params;
        const test = abTests.find(t => t.id === id);
        if (!test) {
 ...
 */
const assignUserToVariant = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const existingParticipant = participants.find(p => p.userId === userId && test.variants.some(v => v.id === p.variantId));
...
exports.assignUserToVariant = assignUserToVariant;
/**
 * ثبت conversion
...
const recordConversion = async (req, res) => {
    try {
        const { id } = req.params;
        const { userId, variantId, metricName = 'conversion', value = 1, metadata = {} } = req.body;
        const test = abTests.find(t => t.id === id);
        if (!test) {
            return res.status(404).json({
                success: false,
        ...
        const participant = participants.find(p => p.userId === userId && p.variantId === variantId);
        if (!participant) {
            return res.status(404).json({
                success: false,
...
const getABTestResults = async (req, res) => {
    try {
        ...
        const bestVariant = results.reduce((best, current) => current.conversionRate > best.conversionRate ? current : best);
        res.json({
            success: true,
            data: {
                testId: test.id,
...
exports.getABTestAnalytics = getABTestAnalytics;
// Helper functions
function generateTimelineData(test) {
    const days = [];
...
function generatePerformanceData(test) {
...
const deleteABTest = async (req, res) => {
    try {
        ...
        participants = participants.filter(p => !deletedTest.variants.some(v => v.id === p.variantId));
        res.json({
            success: true,
