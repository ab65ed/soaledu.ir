    ...
    return (req, res, next) => {
        try {
            ...
            if (error instanceof zod_1.z.ZodError) {
                ...
                return next(new errorHandler_1.ApiError(`خطای اعتبارسنجی: ${errors}`, 400));
            }
            return next(new errorHandler_1.ApiError('خطای اعتبارسنجی نامشخص', 400));
        }
    };
};
...
const schemas = {
    ...
    updateProfile: zod_1.z.object({
        name: zod_1.z.string()
            ...
            .max(50, { message: 'نام نباید بیشتر از ۵۰ کاراکتر باشد' })
            .optional(),
        educationalGroup: zod_1.z.string().optional()
    }),
    // ExamConfig schemas
    createExamConfig: zod_1.z.object({
        category: zod_1.z.string({ message: 'دسته‌بندی الزامی است' }),
        lesson: zod_1.z.string().optional(),
        hasNegativeMarking: zod_1.z.boolean().optional(),
        timerOption: zod_1.z.enum(['50s', '70s', 'none'], {
            errorMap: () => ({ message: 'گزینه تایمر باید یکی از مقادیر 50s، 70s یا none باشد' })
        }).optional()
    }),
    // Answer schemas
    submitAnswer: zod_1.z.object({
        questionId: zod_1.z.string({ message: 'شناسه سوال الزامی است' }),
        selectedOption: zod_1.z.number()
            .int({ message: 'گزینه انتخابی باید عدد صحیح باشد' })
            .min(0, { message: 'گزینه انتخابی نامعتبر است' })
    }),
    // Ticket schemas
    createTicket: zod_1.z.object({
    ...
    addTicketResponse: zod_1.z.object({
        message: zod_1.z.string()
    ...
    createPayment: zod_1.z.object({
        ...
        paymentMethod: zod_1.z.enum(['credit-card', 'paypal', 'bank-transfer', 'other'], {
            errorMap: () => ({ message: 'روش پرداخت نامعتبر است' })
        }),
        packageName: zod_1.z.string().optional()
    }),
    // BlogPost schemas
    createBlogPost: zod_1.z.object({
