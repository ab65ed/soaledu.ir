...
describe('Field of Study Validation Tests', () => {
  describe('FIELD_OF_STUDY Constants', () => {
    test('should contain all expected field of study options', () => {
      expect(FIELD_OF_STUDY).toContain('computer-engineering');
      expect(FIELD_OF_STUDY).toContain('medicine');
      expect(FIELD_OF_STUDY).toContain('pure-mathematics'); // تصحیح: mathematics -> pure-mathematics
      expect(FIELD_OF_STUDY).toContain('law');
      expect(FIELD_OF_STUDY).toContain('math-physics');
      expect(FIELD_OF_STUDY).toContain('other');
    });

    test('should have reasonable number of options', () => {
      expect(FIELD_OF_STUDY.length).toBeGreaterThan(20);
      expect(FIELD_OF_STUDY.length).toBeLessThan(60); // تصحیح: 50 -> 60 (ما 52 رشته داریم)
    });
  });
  ...
  describe('CreateCourseExamSchema with fieldOfStudy', () => {
    ...
    test('should accept valid fieldOfStudy', () => {
      const validData = {
        ...validBaseData,
        fieldOfStudy: 'computer-engineering' as const
      };
    ...
    test('should work with different valid field of study options', () => {
      ...
      testCases.forEach(fieldOfStudy => {
        const validData = {
          ...validBaseData,
          fieldOfStudy: fieldOfStudy as any
        };
  ...
  describe('UpdateCourseExamSchema with fieldOfStudy', () => {
    test('should accept valid fieldOfStudy in update', () => {
      const updateData = {
        fieldOfStudy: 'electrical-engineering' as const
      };
  ...
  describe('Field of Study Categories', () => {
    test('should contain high school fields', () => {
      ...
      highSchoolFields.forEach(field => {
        expect(FIELD_OF_STUDY).toContain(field);
      });
    });
