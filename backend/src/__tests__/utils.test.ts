...
class JWTUtils {
  static generateAccessToken(payload: any): string {
    return jwt.sign(payload, mockEnv.JWT_SECRET, {
      expiresIn: mockEnv.TOKEN_EXPIRY
    } as jwt.SignOptions);
  }

  static generateRefreshToken(payload: any): string {
    return jwt.sign(payload, mockEnv.JWT_REFRESH_SECRET, {
      expiresIn: mockEnv.REFRESH_TOKEN_EXPIRY
    } as jwt.SignOptions);
  }

  static verifyAccessToken(token: string): any {
    return jwt.verify(token, mockEnv.JWT_SECRET);
  }

  static verifyRefreshToken(token: string): any {
    return jwt.verify(token, mockEnv.JWT_REFRESH_SECRET);
  }
...
const questionValidationSchema = z.object({
  title: z.string().min(5, 'عنوان سوال باید حداقل 5 کاراکتر باشد'),
  content: z.string().min(10, 'محتوای سوال باید حداقل 10 کاراکتر باشد'),
  type: z.enum(['multiple-choice', 'true-false', 'short-answer', 'essay']),
  difficulty: z.enum(['easy', 'medium', 'hard']),
  category: z.string().min(1, 'دسته‌بندی الزامی است'),
  lesson: z.string().min(1, 'درس الزامی است')
});
...
class HelperUtils {
  ...
  static formatDate(date: Date, locale: string = 'fa-IR'): string {
...
describe('Utility Functions Tests', () => {
  describe('JWT Utils', () => {
    ...
    it('should throw error for expired token', () => {
      const expiredToken = jwt.sign(mockPayload, mockEnv.JWT_SECRET, { expiresIn: '0s' } as jwt.SignOptions);
