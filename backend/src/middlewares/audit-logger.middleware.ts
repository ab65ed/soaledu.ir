 ...
 */
function determineSeverity(action: string, statusCode: number, method: string): AuditEvent['severity'] {
  // عملیات‌های بحرانی
 ...
 */
function determineCategory(url: string, action: string): AuditEvent['category'] {
 ...
 */
function extractAction(url: string, method: string): string {
 ...
 */
export const startAuditLog = (req: RequestWithUser, res: Response, next: NextFunction) => {
  // تولید شناسه یکتا برای این درخواست
  req.auditEventId = crypto.randomUUID();
  req.startTime = Date.now();
  
  // بررسی اینکه آیا این URL باید monitor شود
  const shouldMonitor = AUDIT_CONFIG.MONITORED_PATTERNS.some(pattern => 
 ...
 */
export const completeAuditLog = (req: RequestWithUser, res: Response, next: NextFunction) => {
  ...
  res.on('finish', () => {
    ...
    const auditEvent: AuditEvent = {
      eventId: req.auditEventId || crypto.randomUUID(),
      timestamp: new Date(),
      userId: req.user?.id,
    ...
    switch (severity) {
      case 'critical':
        ...
        break;
      case 'high':
        logger.warn('High severity audit event', auditEvent);
        break;
      case 'medium':
        logger.info('Medium severity audit event', auditEvent);
        break;
      default:
        logger.debug('Low severity audit event', auditEvent);
    }
 ...
 */
export const logFailedAuth = (req: Request, reason: string, email?: string) => {
 ...
 */
export const logUnauthorizedAccess = (req: Request, resource: string, requiredRole?: string) => {
 ...
 * ثبت تلاش‌های مشکوک
 */
export const logSuspiciousActivity = (req: Request, activityType: string, details: Record<string, any>) => {
