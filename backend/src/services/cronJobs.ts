...
import InstitutionalDiscountGroup from '../models/InstitutionalDiscountGroup';
 ...
 */
export async function expireTimedDiscounts(): Promise<void> {
  try {
    const now = new Date();
    
    // پیدا کردن تخفیف‌های منقضی شده
    const expiredGroups = await InstitutionalDiscountGroup.find({
      isActive: true,
    ...
    const result = await InstitutionalDiscountGroup.updateMany(
      {
        isActive: true,
        endDate: { $lt: now, $exists: true }
      },
 ...
 */
export async function generateDailyDiscountStats(): Promise<void> {
  try {
    const now = new Date();
    ...
    // آمار کلی
    const totalGroups = await InstitutionalDiscountGroup.countDocuments();
    const activeGroups = await InstitutionalDiscountGroup.countDocuments({ isActive: true });
    const expiredGroups = await InstitutionalDiscountGroup.countDocuments({
      isActive: false,
      endDate: { $exists: true, $lt: now }
    });
    const tieredGroups = await InstitutionalDiscountGroup.countDocuments({
      'tiers.0': { $exists: true }
    });
    const timedGroups = await InstitutionalDiscountGroup.countDocuments({
      $or: [
    ...
    const expiringToday = await InstitutionalDiscountGroup.find({
      isActive: true,
      endDate: { $gte: today, $lt: tomorrow }
    });

    // تخفیف‌هایی که امروز شروع می‌شوند
    const startingToday = await InstitutionalDiscountGroup.find({
      isActive: true,
 ...
 */
export async function cleanupInvalidDiscountGroups(): Promise<void> {
  try {
    console.log('🧹 Starting cleanup of invalid discount groups...');

    // پیدا کردن گروه‌هایی که هیچ تخفیفی ندارند
    const invalidGroups = await InstitutionalDiscountGroup.find({
      $and: [
    ...
    if (invalidGroups.length > 0) {
      ...
      await InstitutionalDiscountGroup.updateMany(
        {
          $and: [
            { discountPercentage: { $exists: false } },
            { discountAmount: { $exists: false } },
