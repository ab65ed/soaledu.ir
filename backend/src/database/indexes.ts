 ...
 */
export const discountCodeIndexes = [
  // Code lookup (unique)
 ...
 */
export const institutionIndexes = [
  // Name-based queries
 ...
 */
export const courseExamIndexes = [
 ...
 */
async function applyIndexesToCollection(
  ...
  try {
    ...
    logger.info(`Starting index creation for ${collectionName}`, {
      collectionName,
    ...
    logger.info(`Completed index creation for ${collectionName}`, {
      collectionName
    });

  } catch (error: any) {
    logger.error(`Error applying indexes to ${collectionName}:`, {
      collectionName,
      error: error.message
    });
    throw error;
 ...
 * اعمال تمام index ها
 */
export async function applyAllIndexes(): Promise<void> {
  try {
    logger.info('Starting database optimization with indexes');

    // اعمال index ها به هر collection
    await applyIndexesToCollection('Question', questionIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('User', userIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('Exam', examIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('Category', categoryIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('Transaction', transactionIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('DiscountCode', discountCodeIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('Institution', institutionIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('Wallet', walletIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('CourseExam', courseExamIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('TestExam', testExamIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('Contact', contactIndexes.map(idx => ({ fields: idx.key, options: idx.options })));
    await applyIndexesToCollection('Payment', paymentIndexes.map(idx => ({ fields: idx.key, options: idx.options })));

    logger.info('Successfully applied all database indexes');
...
export async function getIndexStats(collectionName: string): Promise<any> {
  try {
    ...
    logger.error(`Error getting index stats for ${collectionName}:`, {
      collectionName,
      error: error.message
    });
    throw error;
 ...
 */
export async function dropUnusedIndexes(collectionName: string): Promise<void> {
  try {
    const db = mongoose.connection.db;
    const collection = db.collection(collectionName);
    ...
    logger.info(`Index usage stats for ${collectionName}:`, indexStats);

    // این قسمت می‌تواند برای تحلیل استفاده از index ها توسعه یابد
    // در حال حاضر فقط لاگ می‌کند

  } catch (error: any) {
    logger.error(`Error analyzing index usage for ${collectionName}:`, {
      collectionName,
      error: error.message
    });
  }
...
export async function createOptimizedIndexes(): Promise<void> {
  try {
    logger.info('شروع ایجاد ایندکس‌های بهینه‌سازی شده...');

    // User Model Indexes
    const User = mongoose.model('User');
    await User.createIndexes();
    logger.info('ایندکس‌های User model ایجاد شد');

    // Question Model Indexes  
    const Question = mongoose.model('Question');
    await Question.createIndexes();
    logger.info('ایندکس‌های Question model ایجاد شد');

    // Exam Model Indexes
    const Exam = mongoose.model('Exam');
    await Exam.createIndexes();
    logger.info('ایندکس‌های Exam model ایجاد شد');

    // Category Model Indexes
    const Category = mongoose.model('Category');
    await Category.createIndexes();
    logger.info('ایندکس‌های Category model ایجاد شد');

    logger.info('تمام ایندکس‌های بهینه‌سازی شده با موفقیت ایجاد شد');
  } catch (error) {
    logger.error('خطا در ایجاد ایندکس‌ها:', error);
    throw error;
  }
}
 ...
 */
export async function getIndexAnalysis(): Promise<any> {
  try {
    logger.info('شروع تحلیل ایندکس‌های پایگاه داده...');
    ...
    for (const collection of collections) {
      ...
      try {
        ...
        logger.warn(`خطا در دریافت ایندکس‌های کالکشن ${collectionName}:`, error);
        analysis[collectionName] = {
    ...
    logger.info('تحلیل ایندکس‌ها تکمیل شد');
    return analysis;
  } catch (error) {
    logger.error('خطا در تحلیل ایندکس‌ها:', error);
    throw error;
  }
}
...
export async function checkIndexHealth(): Promise<any> {
  try {
    ...
    try {
      ...
      logger.warn('امکان دریافت وضعیت سرور وجود ندارد:', error);
      serverStatus = null;
    }
    ...
    logger.error('خطا در بررسی سلامت ایندکس‌ها:', error);
    throw error;
  }
}
