...
// import { CreateQuestionSchema, QuestionOptionSchema } from './types'; // موقتاً غیرفعال
...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    const uploadPath = path.join(__dirname, '../../../uploads/bulk-questions');
  ...
  filename: function (req, file, cb) {
    ...
    cb(null, 'questions-bulk-' + uniqueSuffix + path.extname(file.originalname));
  }
});
 ...
 */
function parseExcelFile(filePath: string): ExcelQuestionRow[] {
  try {
 ...
 */
async function validateAndProcessRows(
  ...
  for (const row of rows) {
    try {
      ...
      const validatedRow = ExcelQuestionRowSchema.parse(row) as ExcelQuestionRow;
 ...
 */
async function validateQuestionRow(row: ExcelQuestionRow): Promise<{
  isValid: boolean;
  errors: ImportError[];
  warnings: ImportWarning[];
}> {
 ...
 */
async function createQuestionFromRow(
  ...
  if (row.type === 'multiple-choice') {
    if (row.optionA) options.push({ label: 'الف', content: row.optionA, isCorrect: normalizeAnswer(row.correctAnswer) === 'A' });
    if (row.optionB) options.push({ label: 'ب', content: row.optionB, isCorrect: normalizeAnswer(row.correctAnswer) === 'B' });
    if (row.optionC) options.push({ label: 'ج', content: row.optionC, isCorrect: normalizeAnswer(row.correctAnswer) === 'C' });
    if (row.optionD) options.push({ label: 'د', content: row.optionD, isCorrect: normalizeAnswer(row.correctAnswer) === 'D' });
  }
  ...
  try {
    ...
    Object.keys(questionData).forEach(key => {
      if (key !== 'metadata') { // Handle metadata separately
        question.set(key, (questionData as any)[key]);
      }
    });
...
export const bulkUploadQuestions = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await BulkImportService.setProcessing(userId);
...
async function processFileAsync(
  ...
  try {
    ...
    await BulkImportService.saveResult(userId, result);
    ...
    await BulkImportService.saveError(userId, error.message);
  }
}
 ...
 */
export const getBulkUploadStatus = async (req: Request, res: Response): Promise<void> => {
  try {
    const userId = (req as any).user?.id;
    
    // دریافت وضعیت از دیتابیس
    const status = await BulkImportService.getStatus(userId);
