...
interface CachedQuestionSet {
  ...
  usageCount: number;
  lastUsed: Date;
  version: number; // اضافه شده برای versioning
  attemptNumber?: number; // اضافه شده برای attempt tracking
}
...
interface QuestionPoolConfig {
  ...
  attemptNumber?: number; // اضافه شده
  userId?: string; // اضافه شده برای user-specific pools
}
...
export class QuestionCacheService {
  ...
  private questionPools: Map<string, CachedQuestionSet> = new Map();
  private userAttemptHistory: Map<string, UserAttemptHistory> = new Map();
  private readonly CACHE_TTL = 30 * 60 * 1000; // 30 دقیقه
  private readonly MAX_POOLS = 100; // افزایش برای multiple versions
  private readonly POOL_SIZE_MULTIPLIER = 3; // 3 برابر سوالات مورد نیاز
  private readonly MAX_ATTEMPTS = 5; // حداکثر تعداد attempt
  private readonly MIN_POOL_OVERLAP = 0.3; // حداقل 30% تفاوت بین pool ها
   ...
   */
  private generatePoolKey(config: QuestionPoolConfig): string {
   ...
   */
  private generateUserHistoryKey(userId: string, examId: string): string {
   ...
   */
  async getQuestionPoolForAttempt(
  ...
  private async createUniqueQuestionPool(
    config: QuestionPoolConfig, 
    userHistory: UserAttemptHistory
  ): Promise<any[]> {
    ...
    const poolSize = Math.min(
      config.totalQuestions * this.POOL_SIZE_MULTIPLIER * 2, // دو برابر بیشتر برای تنوع
    ...
    const availableQuestions = allQuestionsJson.filter(q => 
      !usedQuestionIds.includes(q._id || q.id)
    );
   ...
   */
  private async getUsedQuestionIds(userId: string, userHistory: UserAttemptHistory): Promise<string[]> {
    const usedIds: string[] = [];
    
    // بررسی pool های استفاده شده قبلی
    for (const version of userHistory.usedPoolVersions) {
      const poolKeys = Array.from(this.questionPools.keys()).filter(key => 
        key.includes(`_v${version}`) && key.includes(userId)
      );
   ...
   * دریافت تاریخچه attempt های کاربر
   */
  private getUserAttemptHistory(userId: string, examId: string): UserAttemptHistory {
    ...
    // ایجاد تاریخچه جدید
    const newHistory: UserAttemptHistory = {
      userId,
      examId,
      attemptCount: 0,
      usedPoolVersions: [],
   ...
   */
  private getRandomSubset(pool: any[], count: number): any[] {
  ...
  private cacheQuestionPool(
    ...
    const cachedSet: CachedQuestionSet = {
      id: key,
      questions,
   ...
   */
  private cleanOldAttemptHistory(): void {
    const now = new Date();
    const expiredKeys: string[] = [];
  ...
  getCacheStats(): {
    ...
    hitRate: number;
    memoryUsage: number;
    mostUsedPools: Array<{key: string, usageCount: number, attemptNumber?: number}>;
    attemptStats: {
      totalUsers: number;
  ...
  async getQuestionPool(config: QuestionPoolConfig): Promise<any[]> {
   ...
   */
  private async getQuestionPoolLegacy(config: QuestionPoolConfig): Promise<any[]> {
   ...
   */
  private async createQuestionPoolLegacy(config: QuestionPoolConfig): Promise<any[]> {
    const query = new Parse.Query(Question);
    query.equalTo('isPublished', true);
    query.equalTo('difficulty', config.difficulty);
