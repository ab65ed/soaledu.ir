...
const verifyPaymentSchema = zod_1.z.object({
    ...
    paymentReference: zod_1.z.string()
});
class PaymentController {
    ...
    static async createPayment(req, res) {
        try {
            ...
            const examQuery = new node_1.default.Query(TestExam);
            ...
            const pricing = await (0, pricing_utils_1.calculateExamPrice)(questionCount, userType, isFirstPurchase, 0);
            // Create transaction record
            const Transaction = node_1.default.Object.extend('PaymentTransaction');
    ...
    static async verifyPayment(req, res) {
        try {
            ...
            const isPaymentValid = await (0, payment_utils_1.verifyWithGateway)(paymentReference);
            if (isPaymentValid) {
                ...
                await (0, payment_utils_1.grantExamAccess)(transaction.get('userId'), transaction.get('examId'));
                // Update user purchase history
                await (0, payment_utils_1.updateUserPurchaseHistory)(transaction.get('userId'), transaction.toJSON());
                res.json({
                    status: 'success',
                    data: {
                        transactionId,
