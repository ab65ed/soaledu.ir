...
exports.findByEnrollmentCode = exports.restoreInstitution = exports.getInstitutionByEnrollmentCode = exports.getInstitutionStudents = exports.getInstitutionStats = exports.deleteInstitution = exports.updateInstitution = exports.createInstitution = exports.getInstitutionById = exports.getInstitutions = void 0;
...
const getInstitutions = async (req, res) => {
    try {
        const { page = '1', limit = '10', search, type, isActive, contractStatus } = req.query;
        ...
        const [institutions, total] = await Promise.all([
            Institution_1.default.find(filter)
                .populate('createdBy', 'name email')
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limitNum)
                .lean(),
            Institution_1.default.countDocuments(filter)
        ]);
...
const getInstitutionById = async (req, res) => {
    try {
        ...
        const institution = await Institution_1.default.findById(id)
            .populate('createdBy', 'name email')
            .lean();
        if (!institution) {
            res.status(404).json({
        ...
        const studentsStats = await user_model_1.default.aggregate([
            { $match: { institutionId: new mongoose_1.default.Types.ObjectId(id) } },
            {
                $group: {
                    _id: null,
                    totalStudents: { $sum: 1 },
        ...
        if (studentsStats.length > 0) {
            ...
            if (institution.totalStudents !== stats.totalStudents ||
                ...
                await Institution_1.default.findByIdAndUpdate(id, {
...
const createInstitution = async (req, res) => {
    try {
        const { name, nameEn, type, description, contactInfo, defaultDiscountSettings, enrollmentSettings, establishedYear, logo, features, contractStartDate, contractEndDate, contractTerms, notes, } = req.body;
        ...
        const existingInstitution = await Institution_1.default.findOne({
        ...
        if (enrollmentSettings?.enrollmentCode) {
            const existingCode = await Institution_1.default.findOne({
...
const updateInstitution = async (req, res) => {
    try {
        ...
        updateData.lastModifiedBy = new mongoose_1.default.Types.ObjectId(req.user._id);
        // بررسی وجود نهاد
        const existingInstitution = await Institution_1.default.findById(id);
        if (!existingInstitution) {
            res.status(404).json({
                success: false,
        ...
        if (updateData.name && updateData.name !== existingInstitution.name) {
            const duplicateName = await Institution_1.default.findOne({
        ...
        if (updateData.enrollmentSettings?.enrollmentCode) {
            ...
            if (newCode !== existingInstitution.enrollmentSettings.enrollmentCode) {
                const duplicateCode = await Institution_1.default.findOne({
        ...
        // به‌روزرسانی نهاد
        const updatedInstitution = await Institution_1.default.findByIdAndUpdate(id, updateData, { new: true, runValidators: true }).populate('createdBy lastModifiedBy', 'name email');
        res.json({
            success: true,
...
const deleteInstitution = async (req, res) => {
    try {
        ...
        const studentsCount = await user_model_1.default.countDocuments({
            institutionId: id,
            role: 'student',
        });
        if (studentsCount > 0) {
        ...
        institution.lastModifiedBy = new mongoose_1.default.Types.ObjectId(req.user._id);
...
const getInstitutionStats = async (req, res) => {
    try {
        const totalInstitutions = await Institution_1.default.countDocuments();
        const activeInstitutions = await Institution_1.default.countDocuments({ isActive: true });
        const inactiveInstitutions = totalInstitutions - activeInstitutions;
        // آمار بر اساس نوع
        const typeStats = await Institution_1.default.aggregate([
            { $match: { isActive: true } },
        ...
        const recentInstitutions = await Institution_1.default.find({ isActive: true })
            .sort({ createdAt: -1 })
            .limit(5)
...
const getInstitutionStudents = async (req, res) => {
    try {
        ...
        const [students, total] = await Promise.all([
            user_model_1.default.find({ institutionId: id })
                ...
                .limit(limitNum)
                .lean(),
            user_model_1.default.countDocuments({ institutionId: id })
        ]);
        const totalPages = Math.ceil(total / limitNum);
        res.status(200).json({
            success: true,
 ...
 */
const getInstitutionByEnrollmentCode = async (req, res) => {
    try {
...
exports.getInstitutionByEnrollmentCode = getInstitutionByEnrollmentCode;
/**
 * بازیابی نهاد (فعال کردن مجدد)
 * PATCH /api/admin/institutions/:id/restore
 */
const restoreInstitution = async (req, res) => {
    try {
        const { id } = req.params;
        const institution = await Institution_1.default.findById(id);
        if (!institution) {
...
exports.restoreInstitution = restoreInstitution;
/**
 * جستجو در نهادها با کد ثبت‌نام
 * GET /api/institutions/search/:enrollmentCode
 */
const findByEnrollmentCode = async (req, res) => {
    try {
        ...
        const institution = await Institution_1.default.findByEnrollmentCode(enrollmentCode);
        if (!institution) {
            res.status(404).json({
                success: false,
...
exports.findByEnrollmentCode = findByEnrollmentCode;
